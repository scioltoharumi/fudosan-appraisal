...
export function renderGuide(r, property, areaCal = null) {
  ...
  const adjRows = [
    ["駅からの徒歩", `${s.walk}分`, pct(m.walkAdj),
      `10分以内は横ばい(±0%)。11分から効きはじめ、13分で−12%、18分以降は−25%で頭打ち(その間は折れ線で補間)。` +
      `${s.walk}分なので ${pct(m.walkAdj)}。台帳479件の実測で11〜15分帯の実勢が−12%だったことに基づく帯別テーブル` +
      `(2026-08-13に「1分あたり−1.2%」の線形から置換。線形式は13分で−3.6%しか引かず実勢の約1/3だった)`],
  ...
  <div class="panel">
    ...
    <div class="logic-body">
      ...
      <p class="why" style="margin-top:8px">この物件は${m.route === "land" ? "ルートA(土地)が上回りました。つまり市場価値の実態は「古家付き土地」です。" : "ルートB(家)が上回りました。建物にまだ market value が残っています。"}また、ルートAの値は<b>下値フロア</b>(最悪でも土地としてこの値では売れるという下限)としても使います: <b>${man(m.floorVal)}</b>。さらに2026-08監査で、この2ルート(原価法)だけでは「住める家」を買う実需市場の支払額を捉えられないことが分かったため、周辺の<b>戸建成約から直接比較する第3のルート(リテール比較法)</b>を追加し、最終的な適正価格は「事例数に応じた重み付き調整(土地換算値を下限保証)」で決めています(第2次監査でmax採用の上振れバイアスを指摘され修正)${r.retail ? `(この物件は類似成約${r.retail.n}件からリテール値 ${man(r.retail.mid)} が得られ、${r.fairFinal.route === "retail" ? "これが採用されています" : "原価法側が上回っています"})` : ""}。</p>
    </div>
  </div>
  ...
  <div class="panel">
    <h2>STEP 9 ── 参照水準の読み方(この台帳は判定をしない)</h2>
    <div class="logic-body">
      <p class="why">本台帳は<b>「買い」「見送り」といった判定を出しません</b>。同じ数字を見ても、住む本人の事情(通勤・家族構成・住宅ローンの余力・いつまでに決めたいか)で結論は変わるからです。エンジンが出すのは<b>売出価格が各参照水準のどこに立っているか</b>という事実までで、そこから先は人が決めます(検討状況は<a href="index.html">一覧ページ</a>で自分で設定できます)。読むべき参照水準は次の4つです。</p>
      <table class="kv">
        <tr><td><b>即時処分値</b></td><td>土地値×卸値90%×売却諸費用控除 − 解体費。業者に土地として即売りしたときの手取り相当。<b>取得総額(価格×1.05)がこれを下回るなら、最悪ケースでも取得額を回収できる位置</b>にあることを意味します</td></tr>
        <tr><td><b>土地換算値(下値フロア)</b></td><td>古家を壊して土地として売る前提の価値。売出価格との差は「建物の残り価値」「周辺の戸建成約水準」「土地の上振れ期待」のいずれかに払っている部分です</td></tr>
        <tr><td><b>市場実勢中央値 / 上位四分位</b></td><td>周辺の戸建成約から時点・徒歩・築年差を補正した実需市場の水準。<b>中央値との差が売主の期待</b>、上位四分位に通常の交渉幅5%を載せた水準が「通常交渉で届く上限」の目安です</td></tr>
        <tr><td><b>適正中央値(原価法との重み付き)</b></td><td>市場実勢と原価法を事例数で重み付けした値。市場実勢がこれを上回る分は<b>市場全体の過熱感=相場が冷えたときの下落余地</b>で、物件の良し悪しではありません</td></tr>
      </table>
      <div class="formula" style="margin-top:10px">この物件: 売出 ${man(s.ask)}${r.negoBand ? ` / 通常交渉で届く上限の目安 ${man(Math.round(r.negoBand))}(上位四分位+交渉幅5%)` : ` / 適正レンジ楽観上限 ${man(Math.round(r.fairFinal.hi))}`} / 市場実勢との差 ${(r.premiumMarket ?? r.premium) >= 0 ? "+" : ""}${man(r.premiumMarket ?? r.premium)}</div>
      <p class="why">なお売出価格と査定が乖離していること自体は珍しくありません。売主の希望価格・住宅ローン前提の実需価格・リフォーム済プレミアムなどが上乗せされるためです。乖離の大きさは「この売主が今いくら期待しているか」を示す数字であり、それを払うかどうかは条件との引き換えで決める話になります。</p>
    </div>
  </div>
  ...
  <div class="panel">
    ...
    <div class="logic-body">
      <table class="kv">
        ...
        <tr><td><b>再調達単価</b></td><td>同じ建物を今新築したらかかる坪単価(本エンジンは${COEFFS.DEFAULT_REBUILD_PPT}万/坪)</td></tr>
        <tr><td><b>繰延修繕</b></td><td>先送りされている修繕。買った直後に払うことになる隠れコスト</td></tr>
        <tr><td><b>土地換算値/即時処分値</b></td><td>土地値−解体費が土地換算値。そこから業者卸値(×90%)と売却諸費用(4%)を引いたものが即時処分値で、資産防衛の下限を見る水準</td></tr>
  ...
  return layout({
    ...
    subtitle: `題材: ${esc(addr)} ── 公示地価から参照水準まで、数字の出所を一つずつ`,

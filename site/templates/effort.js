...
function figTimeline() {
  ...
  el.push(`<text x="${(X(0.6)).toFixed(1)}" y="172" font-size="8.5" fill="#2C6E49">〜10年目: 保証の傘(瑕疵担保+設備保証=発注が少ない)</text>`);
  baseline(bN);
  steadyBand(bN);
...
export function renderEffort({ asOf }) {
  ...
  <section class="panel">
    ...
    <b>新築</b>は最初の10年が保証の傘(構造・雨漏りの瑕疵担保10年+設備のメーカー保証)で静かで、発注者業が始まるのは11年目以降。
  ...
  <section class="panel">
    ...
    故障の発注が本格化するのは11年目以降。築15〜21年の中古は入口からこのリストが現役で回る——

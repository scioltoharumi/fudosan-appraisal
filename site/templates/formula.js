...
export function ageCurveCI(houseDeals, resamples = 4000) {
  ...
  const rows = buckets.filter((b) => b.ratios.length > 0).map((b) => {
    ...
    const verdict = hi95 < 1 ? `ものさしより安い${thin}` : lo95 > 1 ? `ものさしより高い${thin}` : `ものさしどおりの範囲(1.00を含む)${thin}`;
...
function ageCurveSvg(ac) {
  ...
  el.push(`<text x="${padL}" y="${H - 6}" font-size="9" fill="#43566B">縦軸: 実際の成約価格 ÷ ものさし価格(地区の土地代 + 築年数ぶん目減りさせた建物代)。1.0 = ものさしどおりの値段</text>`);
  return `<svg viewBox="0 0 ${W} ${H}" role="img" aria-label="築年帯別の 成約価格÷ものさし価格: 築30年までフラット、以降に崖" style="width:100%;height:auto">${el.join("")}</svg>`;
}
...
本物件(築${s.age.toFixed(1)}年・未修繕) = ${fmtMan(Math.round(rt.bldgSubj))} / 修繕・リフォーム済みなら ${fmtMan(Math.round(rt.bldgSubj + (rt.repairSubj ?? 0)))}(差 ${fmtMan(Math.round(rt.repairSubj ?? 0))})</pre>
  ...
  <div class="panel">
    ...
    <div class="logic-body">
      <p class="why">「新築は買った瞬間に2割下がる」という話を、台帳の成約データで確かめる。やり方はこうだ。まず地区ごとに<b>土地の相場単価</b>を決める(その地区の築11〜25年の成約から、建物ぶんを引いた残りを土地の値段とみなす)。次に1件ずつ、比べるための<b>ものさし価格</b>を出す ── <b>「その地区の土地代 + 建物代を築年数ぶん目減りさせた残り」</b>。建物は新築時95万円/坪で、30年かけて真っ直ぐゼロに向かう、という単純な引き算だ。この足し算・引き算だけで出た値段を、以下「ものさし価格」と呼ぶ。<br>
      最後に<b>実際の成約価格 ÷ ものさし価格</b>を築年別に並べる。<b>1.00なら「ものさしどおりの値段で売れている」</b>、1.00より大きければものさしより高く、小さければ安く売れている、という意味になる。もし新築に上乗せ(プレミアム)が実在するなら、築0〜3年だけが1.00を大きく超えるはずだ。</p>
      ${ageCurveSvg(ac)}
      <div class="note"><b>結果の読み方</b>: 結論から言うと<b>新築だけ高い、という山は立たなかった</b>。築0〜3年は${ac.buckets[0].ratio.toFixed(2)}で、ものさしとほぼ同じか、むしろ気持ち安いくらい。中古も築30年あたりまで1.00の周りに並ぶ。<br>
      <b>はっきり違うのは築31年から先だけ</b>だ(${acCliff.ratio.toFixed(2)}→${acOld.ratio.toFixed(2)})。この帯ではものさしの建物代はもうほぼゼロなので、比率${acOld.ratio.toFixed(2)}は「<b>土地の値段そのものが2〜4割引きでしか売れていない</b>」ことを意味する。買った人は古家を壊す前提なので、解体費と手間のぶん土地を買い叩く。つまり築30年を越えると、建物を失うだけでなく<b>土地まで値引きされる</b>。<br>
      ...
      逆に<b>築7〜30年の細かい起伏は、標本が薄く判別できない</b>(どの帯も信頼区間が1.00を含む)。とくに築26〜30年は件数が少なく事実上情報がない。したがって「築15年で建物価値が何%残るか」をこの台帳で断定はできず、言えるのは<b>「築30年まではものさしと矛盾しない」</b>までだ。崖が始まるのが築31年ちょうどなのか、もっと手前なのか後なのかも特定できていない。<br>
      <b>2026-08-13の変化</b>: 探索エリアを王子・上中里方面へ広げて6地区286件を足したところ(479→765件)、築3〜7年帯だけが1.00を含まなくなった(区間の上限が1.00未満)。ただし足した6地区は近隣商業・準工業の混入が既存12地区より多く、<b>市場の事実なのか母集団の構成が変わっただけなのかは切り分けられていない</b>。この帯を根拠に何かを判断するのは早い。

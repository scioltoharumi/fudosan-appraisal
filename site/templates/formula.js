...
本物件(築${s.age.toFixed(1)}年・未修繕) = ${fmtMan(Math.round(rt.bldgSubj))} / 修繕・リフォーム済みなら ${fmtMan(Math.round(rt.bldgSubj + (rt.repairSubj ?? 0)))}(差 ${fmtMan(Math.round(rt.repairSubj ?? 0))})</pre>
  ...
  <div class="panel">
    ...
    <div class="logic-body">
      ...
      逆に<b>築3〜30年の細かい起伏は、標本が薄く判別できない</b>(どの帯も信頼区間が1.00を含む)。とくに築26〜30年は6件しかなく事実上情報がない。したがって「築15年で建物価値が何%残るか」をこの台帳で断定はできず、言えるのは<b>「築30年まではものさしと矛盾しない」</b>までだ。崖が始まるのが築31年ちょうどなのか、もっと手前なのか後なのかも特定できていない。

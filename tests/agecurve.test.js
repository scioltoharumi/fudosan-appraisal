...
test("築年カーブ: 回帰値(成約/モデル比の築年帯別中央値・2026-08-13時点の台帳)", () => {
  ...
  const expected = [
    [0, 0.917, 249], [3, 0.861, 28], [7, 1.002, 32], [11, 1.000, 37],
    [16, 1.000, 36], [21, 1.000, 23], [26, 1.059, 12], [31, 0.760, 40], [41, 0.566, 94],
  ];
...
test("築年カーブCI: 崖は有意(差の95%区間が0を跨がない)・新築に上乗せの山は無い", () => {
  ...
  assert.ok(Math.abs(ci.cliffDiff - 0.341) < 0.01, `崖の差: ${ci.cliffDiff.toFixed(3)}`);
  // 標本が320→551に増えて区間は狭まった(0.258〜0.426 → 0.269〜0.377)。崖の主張は強くなっている

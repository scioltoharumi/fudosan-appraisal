...
test("delisted_observed があるページには掲載終了の疑いが日付つきで出る", () => {
  const pages = Object.fromEntries(renderAll());
  // 対象物件は日次クロールで入れ替わる(5日超ルールで台帳から外れる)。2026-09-25時点は上十条4 古家付き+参考プラン(SUUMO・athomeとも404・9/24起点)
  const body = visibleText(pages["property/kamijujo4-21403138.html"]);
  assert.ok(body.includes("掲載終了の疑い"), "見出しが無い");
  assert.ok(body.includes("2026-09-24"), "観測日が無い");
  ...
  const row = idx.split("<tr class=\"prow\"").find((s) => s.includes('data-id="kamijujo4-21403138"'));
  assert.ok(row && row.includes("掲載終了疑い(2026-09-24)"), "一覧の当該行に印が無い");

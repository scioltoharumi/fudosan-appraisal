...
// ---- KO2の誤爆: athomeの空値ラベル「借地期間・地代」(2026-08-30) ----
// athomeの概要ヘッダは所有権物件でも「借地期間・地代 （月額） － 権利金 －」と空値ラベルを常に出す。
// アピールポイントの窓(600字)は本文が短いとこのヘッダまで届き、「借地期間」がLEASEHOLD_REに
// 誤爆する。実害: at_1107592414(赤羽台3・新築・土地権利「所有権」明記)がKO2でブロックされ、
// 希望条件を満たす候補が16日間ではなく恒久に埋もれるところだった(ko_blockedは再審査されない)。
// screen.mjs冒頭の「ラベル語だけで旗を立てない」の実害例。
test("KO2: athomeの空値ラベル「借地期間・地代 （月額） －」に誤爆しない", () => {
  // 実ページの構造を再現: アピールポイント本文が短く、窓(600字)が概要ヘッダの空値ラベルへ届く
  const html = `<div>アピールポイント 閑静な住宅地内の新築戸建です。駅徒歩8分。
    交通 ＪＲ埼京線 / 赤羽駅 徒歩8分 所在地 東京都北区赤羽台３丁目 物件種目 新築一戸建て
    価格 7,280 万円 支払額シミュレーション 借入可能額シミュレーション
    借地期間・地代 （月額） － 権利金 － 敷金・保証金 － / － 維持費等 － その他一時金 なし
    土地権利 所有権 都市計画 市街化区域 接道状況 西 3.2m 私道 接面9.3m
    土地面積 62.61m² 建物面積 90.25m²</div>`;
  const scan = scanKO(html, "athome");
  assert.ok(!scan.flags.some((f) => f.code === "KO2_ownership"),
    "空値の借地ラベルだけでKO2が立っている(所有権明記の物件を落とす)");
  assert.equal(scan.attrs.ownership, "所有権");
  // 値が入っているラベルは従来どおりKO2(at_1127269513=旧法賃借権・地代27,000円/月)
  const lease = scanKO(html.replace("借地期間・地代 （月額） －", "借地期間・地代 （月額） 20年 27,000円")
    .replace("土地権利 所有権", "土地権利 旧法賃借権"), "athome");
  assert.ok(lease.flags.some((f) => f.code === "KO2_ownership"), "実値つき借地ラベルがKO2で拾われない");
  // evidence は実際の根拠を出す(own=所有権のまま語彙で立つと「所有権」と表示される矛盾があった)
  const evi = lease.flags.find((f) => f.code === "KO2_ownership").evidence;
  assert.ok(/賃借権|借地期間/.test(evi), `evidenceが根拠を示していない: ${evi}`);
});

// 2026-09-25: 窓(600字)の末尾がラベルの途中「借地期間・地」で切れると、窓を切った後の scrub が
// 全体形を認識できず KO2 が立っていた(at_1196096521=滝野川2・土地権利「所有権」の新築)。scrub は窓の前に掛ける
test("KO2: 窓の末尾で「借地期間・地|代 （月額） －」が切れても誤爆しない", () => {
  const html = `<div>アピールポイント${"あ".repeat(586)}借地期間・地代 （月額） － 権利金 － 土地権利 所有権</div>`;
  const scan = scanKO(html, "athome");
  assert.ok(!scan.flags.some((f) => f.code === "KO2_ownership"), "窓で切れた空値ラベルでKO2が立っている");
});

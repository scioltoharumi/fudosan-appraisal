...
export function buildExcludedIndex(excluded) {
  ...
  for (const [key, v] of Object.entries(excluded ?? {})) {
    ...
    sites.push({
      ...
      // 2026-09-07: 「宅地造成」は北区全域の法令表記なのでハザード現場の識別語から外した(HAZARD_RE と同じ理由)
      hazard: /土砂災害|がけ条例|浸水|警戒区域|ハザード/.test(String(v.reason ?? "")),
...
// 2026-09-07(ユーザー決定「北区全域ならノックアウトはおかしい」): **宅地造成工事規制区域は語彙から外した**。
// 盛土規制法により令和6年7月31日から**北区全域**が宅地造成等工事規制区域になっており(北区公式ページで確認)、
// 掲載の「宅地造成工事規制区域」表記は区画固有のハザードではなく区全域に当たる法令表記。旧法(宅地造成等規制法)の
// 規制区域は都内13市区(世田谷・板橋・八王子…)で北区は含まれず、この語が区画の崖・造成を意味した時期は北区には無い。
// 実害: 岸町2 MIRASUMO(HIT_ALLOWED例外)・滝野川2 nc_20708232/nc_21342883・上十条4 nc_21403138 がこの語だけでKO4になっていた。
// がけ条例・土砂災害警戒区域・急傾斜地・浸水想定は区画固有の指定なので従来どおりKO4
const HAZARD_RE = /土砂災害特別警戒区域|土砂災害警戒区域|急傾斜地崩壊|がけ条例|建築安全条例第[6六]条|浸水想定|洪水浸水/;
...
export function parseDetailAttrs(html, media) {
  ...
  // 2026-09-19: 「参考プランのご相談…お気軽にお問い合わせください！ 土地価格6990万円」(滝野川2 nc_21733272 の営業文)が
  // 参考プランの窓に掛かり、**土地価格そのものを建物価格として読んで総額13,980万**にしていた。窓の中に「土地価格」「価格 ヒント」が
  // 現れたらそこで窓を閉じる(参考プランの金額は「参考プラン有（77.62m2、1500万円）」のように同じ句の中に出る)
  const bldgPrices = [...t.matchAll(/建物価格\s*(?:ヒント\s*)?([0-9,]{3,7})\s*万円/g),
    ...t.matchAll(/参考プラン(?:(?!土地価格|価格\s*ヒント)[^。]){0,40}?([0-9,]{3,7})\s*万円/g)]

...
// 2026-09-20: 72→120。台帳が46物件に増え、watch(source_url 1物件=1頁)だけで44頁を使うため、一覧(SUUMO 中古7+新築8+土地6頁・athome 中古3+新築4頁≒28頁)と
// 詳細取得の取り置き8を足すと80頁を超え、9/9以降ほぼ毎日 athome 新築一覧や土地一覧の途中で打ち切られていた(9/20は土地一覧6頁目で打ち切り=新着の取りこぼし)。
// 間隔2.2秒は http.mjs で不変なので、増えるのは1回のクロールの所要時間(約2.6分→4.4分)だけ。上限は環境変数 PAGE_BUDGET で上書き可
const MAX_PAGES = Number(process.env.PAGE_BUDGET ?? 120) + Math.max(0, Number(process.env.KO_DETAIL_MAX ?? 8) - 8);
